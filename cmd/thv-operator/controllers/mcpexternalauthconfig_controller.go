...
func (r *MCPExternalAuthConfigReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	mcpServerHandler := handler.EnqueueRequestsFromMapFunc(
		func(ctx context.Context, obj client.Object) []reconcile.Request {
			// When an MCPServer changes, enqueue all MCPExternalAuthConfigs in the same namespace
			// This ensures we catch changes in references
			mcpServer, ok := obj.(*mcpv1alpha1.MCPServer)
			if !ok {
				return nil
			}

			// List all MCPExternalAuthConfigs in the same namespace
			externalAuthConfigList := &mcpv1alpha1.MCPExternalAuthConfigList{}
			if err := r.List(ctx, externalAuthConfigList, client.InNamespace(mcpServer.Namespace)); err != nil {
				log.FromContext(ctx).Error(err, "Failed to list MCPExternalAuthConfigs")
				return nil
			}

			// Create reconcile requests for each MCPExternalAuthConfig
			requests := make([]reconcile.Request, 0, len(externalAuthConfigList.Items))
			for _, config := range externalAuthConfigList.Items {
